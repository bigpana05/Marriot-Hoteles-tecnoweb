...
export class CompleteBookingComponent implements OnInit, OnDestroy {
  ...
  applyCoupon(): void {
    ...
    if (!this.guestInfo.email) {
      this.showCouponError('Por favor ingresa tu correo electrónico primero para validar el cupón.');
      return;
    }
    ...
    this.couponService.getAll().subscribe({
      next: (coupons) => {
        ...
        this.verifyCouponHistory(this.guestInfo.email, codeToVerify, foundCoupon);
      },
      error: () => this.showCouponError('Error al verificar el cupón.')
    });
  }

  private verifyCouponHistory(email: string, code: string, coupon: Coupon): void {
    this.bookingService.getBookingsByEmail(email).subscribe({
      next: (bookings) => {
        ...
        const alreadyUsed = bookings.some(b => 
          (b as any).couponCode === code && b.status !== 'CANCELLED'
        );

        if (alreadyUsed) {
          this.showCouponError('Ya has utilizado este cupón en una reserva anterior.');
        } else {
          // Si pasa todas las pruebas, aplicamos el descuento
          this.calculateDiscount(coupon);
        }
      },
      error: () => this.showCouponError('No se pudo verificar tu historial de cupones.')
    });
  }
  ...
  registerMember(): void {
    ...
    if (!this.guestInfo.firstName || !this.guestInfo.lastName || !this.guestInfo.email) {
      ...
      setTimeout(() => { this.showRegistrationError = false; }, 4000);
      return;
    }
    const userData = {
    ...
    this.authService.register(userData)
      .then(newUser => {
        ...
        setTimeout(() => { this.showRegistrationSuccess = false; }, 5000);
      })
      .catch(error => {
        this.showRegistrationError = true;
